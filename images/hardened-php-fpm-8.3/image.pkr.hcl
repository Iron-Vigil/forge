...
source "docker" "hardened_php_fpm_83" {
...
build {
  sources = ["source.docker.hardened_php_fpm_83"]
  ...
  provisioner "file" {
    source      = "${path.root}/../../components/php-fpm-8.3/www.conf"
    destination = "/tmp/if_php_www.conf"
  }

  # PHP-FPM 8.3 component
  provisioner "shell" {
    script = "${path.root}/../../components/php-fpm-8.3/install.sh"
  }
