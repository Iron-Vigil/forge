...
source "docker" "hardened_php_fpm_85" {
...
build {
  sources = ["source.docker.hardened_php_fpm_85"]
  ...
  provisioner "file" {
    source      = "${path.root}/../../components/php-fpm-8.5/www.conf"
    destination = "/tmp/if_php_www.conf"
  }

  # PHP-FPM 8.5 component
  provisioner "shell" {
    script = "${path.root}/../../components/php-fpm-8.5/install.sh"
  }
